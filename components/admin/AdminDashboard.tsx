                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {sortedOrders
                                    ...
                                    .map(order => {
                                        ...
                                        return (
                                            <div key={order.codigo || order.id.slice(0, 5)} className="group bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all border-l-4 border-l-yellow-400 flex flex-col relative overflow-hidden">
                                                <div className="absolute top-0 right-0 w-24 h-24 bg-yellow-50 rounded-bl-full -mr-10 -mt-10 z-0"></div>
                                                <div className="relative z-10">
                                                    <div className="flex justify-between items-start mb-4">
                                                        <span className="font-mono text-4xl text-gray-900 font-black tracking-tighter shadow-sm bg-white/50 backdrop-blur-sm px-2 rounded-lg -ml-2">
                                                            #{order.codigo || order.id.slice(0, 5)}
                                                        </span>
                                                        <div className="flex flex-col items-end gap-1">
                                                            <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-yellow-100 text-yellow-700 shadow-sm border border-yellow-200">
                                                                {order.estado}
                                                            </span>
                                                            <span className="text-[10px] text-gray-400 font-medium flex items-center gap-1 bg-white px-2 py-0.5 rounded-full shadow-sm border border-gray-100">
                                                                <Clock size={10} />
                                                    ...
                                                    <h3 className="font-bold text-xl text-gray-900 mb-1 flex items-center gap-2">
                                                        {order.cliente}
                                                        {isLoyal && (
                                                            <span title={`Cliente Fiel (${clientOrderCount} pedidos)`} className="bg-gradient-to-r from-yellow-200 to-yellow-100 text-yellow-800 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1 border border-yellow-300 shadow-sm">
                                                                <Users size={12} /> Fiel
                                                            </span>
                                                        )}
                                                    </h3>
                                                    ...
                                                    <div className="bg-gray-50 p-4 rounded-xl mb-4 border border-gray-100 shadow-inner">
                                                        <div className="space-y-2">
                                                            {order.items.map((item, i) => (
                                                                <div key={i} className="flex justify-between items-center text-sm border-b border-gray-200 last:border-0 pb-1 last:pb-0 border-dashed">
                                                                    <div className="flex items-center gap-2">
                                                                        <span className="bg-white text-gray-900 font-bold px-2 py-0.5 rounded border border-gray-200 text-xs shadow-sm">
                                                                            {item.cantidad % 1 === 0 ? item.cantidad : item.cantidad.toFixed(1).replace('.', ',')}x
                                                                        </span>
                                                        ...
                                                        {order.motivoAudit && (
                                                            <div className="mt-3 text-[10px] italic text-red-600 bg-red-50 p-2 rounded-lg border border-red-100 flex items-start gap-1">
                                                                <AlertCircle size={12} className="mt-0.5 shrink-0" />
                                                                <span>Nota: {order.motivoAudit}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="flex justify-between items-end pt-2 mb-4">
                                                        <div>
                                                            ...
                                                            <div className="text-3xl font-black text-gray-900 tracking-tight">${order.total.toLocaleString()}</div>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            <button
                                                                onClick={() => { setEditingOrder(order); setIsEditModalOpen(true); }}
                                                                className="p-2 bg-white border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 hover:border-gray-300 shadow-sm transition-all active:scale-95"
                                                                title="Editar Pedido"
                                                            >
                                                                <Edit size={18} />
                                                            </button>
                                                            <button
                                                                ...
                                                                className="p-2 bg-white border border-red-100 text-red-500 rounded-xl hover:bg-red-50 hover:border-red-200 shadow-sm transition-all active:scale-95"
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3 mt-auto pt-4 border-t border-dashed border-gray-200">
                                                        <button
                                                            onClick={() => handleStatusChange(order.id, 'PAGADO EFECTIVO')}
                                                            className="py-3 rounded-xl text-xs font-black uppercase bg-green-50 text-green-700 hover:bg-green-100 border border-green-200 hover:border-green-300 shadow-sm transition-all active:scale-95 flex items-center justify-center gap-2"
                                                        >
                                                            <DollarSign size={16} /> Efectivo
                                                        </button>
                                                        <button
                                                            onClick={() => handleStatusChange(order.id, 'PAGADO TRANSF')}
                                                            className="py-3 rounded-xl text-xs font-black uppercase bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 hover:border-blue-300 shadow-sm transition-all active:scale-95 flex items-center justify-center gap-2"
                                                        >
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead className="bg-gray-50/50 border-b border-gray-200">
                                            <tr>
                                                ...
                                                <th className="p-5 text-xs font-bold text-gray-500 uppercase tracking-wider">Código</th>
                                            ...
                                            {sortedOrders
                                                ...
                                                .map(order => (
                                                    <tr key={order.id} className="group hover:bg-gray-50 transition-colors">
                                                        ...
                                                        <td className="p-5">
                                                            <span className="font-mono font-bold text-gray-700 bg-gray-100 px-2 py-1 rounded-lg text-sm border border-gray-200">
                                                                #{order.codigo || order.id.slice(0, 5)}
                                                            </span>
                                                        </td>
                                                        <td className="p-5">
                                                            <div className="flex flex-col">
                                                                <span className="font-bold text-gray-900">{order.cliente}</span>
                                                                <span className="text-xs text-gray-400 font-medium">{order.telefono}</span>
                                                            </div>
                                                        </td>
                                                        <td className="p-5">
                                                            <div className="flex flex-col gap-1 max-w-md">
                                                                {order.items.slice(0, 3).map((item, i) => (
                                                                    <div key={i} className="flex items-center gap-2 text-xs text-gray-600">
                                                                        <span className="font-bold bg-gray-100 px-1.5 py-0.5 rounded text-gray-800 text-[10px]">
                                                                            {item.cantidad % 1 === 0 ? item.cantidad : item.cantidad.toFixed(1).replace('.', ',')}x
                                                                        </span>
                                                        ...
                                                        <td className="p-5 text-center">
                                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm border ${order.estado === 'CANCELADO'
                                                                ? 'bg-gray-100 text-gray-500 border-gray-200'
                                                                : order.estado.includes('EFECTIVO') ? 'bg-green-50 text-green-700 border-green-200' : 'bg-blue-50 text-blue-700 border-blue-200'
                                                                }`}>
                                                                <span className={`w-1.5 h-1.5 rounded-full ${order.estado === 'CANCELADO' ? 'bg-gray-400' : 'bg-current'}`}></span>
                                                    ...
                                                    <tr>
                                                        <td colSpan={7} className="p-12 text-center">
                                                            <div className="flex flex-col items-center justify-center text-gray-400">
                                                                ...
                                                                <p className="text-lg font-medium text-gray-500">No hay pedidos en esta sección</p>
                                                                <p className="text-sm">Los pedidos aparecerán aquí cuando cambien de estado.</p>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                )}
